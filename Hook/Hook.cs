...
using static Hook.Base.Extensions;
using System.Linq;
...
namespace Hook.Base
{
    ...
    public abstract unsafe class Hook
    {
        ...
        public void Compile(bool ImportHook = false)
        {
            ...
            if (Function == null)
                throw new EntryPointNotFoundException(Export ?? Ordinal.ToString());
        ...
        private void SetupImportHook() {
            var Imports = GetModuleImports((byte*)Process.GetCurrentProcess().MainModule.BaseAddress);
        ...
        public unsafe static ImportEntry[] GetModuleImports(byte* Module)
        {
            ...
            while (true)
            {
                ...
                while (true)
                {
                    ...
                    if (!ImportByOrdinal)
                        ExportName = Marshal.PtrToStringAnsi(new IntPtr((void*)((ulong)EntryPtr + 2)));
